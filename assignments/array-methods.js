...
let fullName = [];
let bothNames = runners.forEach(name => {
...
let ticketPriceTotal = runners.reduce((total, name) => {
  return (total += name.donation);
}, 0);
...
// Problem 1 - All Last Names in Uppercase

let allLowerCase = runners.map(name => {
  return name.last_name.toLowerCase();
});

console.log(allLowerCase);

// Problem 2 - Find the average donation - Use ticketPriceTotal in answer

let averageDonation = ticketPriceTotal / runners.length;
console.log(averageDonation);

// Problem 3 - Add all donations greater than the average

let largeDonations = runners.filter(donation => {
  if (donation.donation > averageDonation) {
    return true;
  }
});

// Also used in Problem 4
let myDonationFunction = (total, name) => {
  return (total += name.donation);
};

let totalLargeDonations = largeDonations.reduce(myDonationFunction, 0);

console.log(totalLargeDonations);

// Problem 4 - Add all donations less than the average donation.

let smallDonations = runners.filter(donation => {
  if (donation.donation < averageDonation) {
    return true;
  }
});

let totalSmallDonations = smallDonations.reduce(myDonationFunction, 0);
